...
//print_r($request->all());
                ...
                if ($studentId != 0) {
                    ...
                    $this->data['studentDetails'] = DB::table('tbl_students AS si')
                            ->select('si.section_id', 'si.institute_branch_version_id', 'si.student_id', 'si.system_generated_student_id',
							'si.student_name', 'si.roll_no', 'ci.id AS class_id', 'ci.ClassName'/* , 'sci.section_id', 
							'sci.SectionName' */, 'sg.id AS group_id', 'sg.GroupName', 'si.stu_group')
...
//                        ->update($paymentConfirmArr);
				...
				//echo '<pre>';print_r($received_time );echo '</pre>';exit();
...
//                return redirect()->route('generateStudentPaymentSlip::viewOrPrint', array('generateType' => 'View', 'studentId' => $request->system_generated_student_id, 'generateMonth' => $request->generated_month, 'receiptNo' => 0));
    ...
    public function generate_month_wise_student_payment_slip(Request $request) {
        ...
        $cash_in_hands  = DB::table('tbl_students')
						...
						->where('student_id', $student_id)
						->first();
        $free_id  = DB::table('tbl_students')
            ->select('free_payment_view_id')
            ->where('system_generated_student_id', $studentId)
            ->where('student_id', $student_id)
            ->first();
        $comma_seperator = explode(',', $free_id->free_payment_view_id);
        
        //echo '<pre>';var_dump($comma_seperator);echo '</pre>';exit();
        


	    $free_payment  = DB::table('tbl_stu_free_payment_view')
						->select('head_id','amount')
						//->where('system_generated_student_id', $studentId)
						->whereIn('free_payment_view_id', $comma_seperator)
						->get();
        //echo '<pre>';var_dump($free_payment);echo '</pre>';exit();
            
            $free_payment_ids = array(0 => '0');
            $free_payment_amounts = array(0 => '0');
            $TotalFreePayment = count($free_payment);
            for($start = 1; $start <= $TotalFreePayment; $start++){
                $free_payment_ids[$start] = $free_payment[$start - 1]->head_id;
                $free_payment_amounts[$start] = $free_payment[$start - 1]->amount;
            }
            
            //echo '<pre>';var_dump($free_payment_ids);echo '</pre>';exit();
//            $free_payment_head_id = 0;
//            $free_payment_amount = 0;
//            if(!is_null($free_payment)){
//                $free_payment_head_id = $free_payment->head_id;
//                $free_payment_amount = $free_payment->amount;
//            }
        ...
        if (!empty($previous_payment_details)) {
            ...
            $i = 0;
            echo '<div class="table-responsive"><table class="table table-condensed table-bordered text-uppercase">
...
echo '<script type="text/javascript">document.getElementById("availableCashInHands").innerHTML = "<strong>' . $cash_in_hands->cash_in_hands . '</strong>";document.getElementById("tempCashInHands").value = "' . $cash_in_hands->cash_in_hands . '";</script>';
                ...
                foreach ($monthWiseHeads as $mwh) {
                    ...
                    if($key = array_search($mwh->student_payment_id, $free_payment_ids)){
                        $temp_payment_amount = $free_payment_amounts[$key];
                    }

                    echo '<tr class="active">';
                    echo '<td>' . ++$i . '</td>';
                    echo '<td>' . $mwh->student_payment_name . '</td>';
                    echo '<td>' .($mwh->payment_amount-$temp_payment_amount). '</td>';
                    echo '<td><div class="checkbox">
    <label>
      <input type="checkbox" name="received_flag_by_school[]" value="' . $mwh->student_payment_id . '_' . ($mwh->payment_amount-$temp_payment_amount) . '"> &nbsp;&nbsp;
    </label>
  </div></td>';
                    echo '<input type="hidden" name="student_payment_name[]" value="' . $mwh->student_payment_name . '">';
                    echo '<input type="hidden" name="student_payment_id[]" value="' . $mwh->student_payment_id . '">';
                    echo '<input type="hidden" name="payment_amount[]" value="' . ($mwh->payment_amount-$temp_payment_amount) . '">';
                    //echo '<input type="hidden" name="month[]" value="' . $month . '">';
                    echo '</tr>';
                    //echo '<input type="hidden" name="received_flag_by_school[]" value="' . $mwh->student_payment_id . '_' .($mwh->payment_amount-$temp_payment_amount) . '">';
                }
                echo '<tfoot>';
                echo '<tr>';
					echo '<td colspan="2" class="text-right text-bold">Total :</td>';
