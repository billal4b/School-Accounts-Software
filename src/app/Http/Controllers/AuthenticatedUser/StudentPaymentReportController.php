...
class StudentPaymentReportController extends Controller {
     ...
     */
      public function index() {
		...
		//$this ->data['students'] = DB::table('tbl_students')->count();
		//$students = DB::table('tbl_students')->count();
	...
	public function confirm_bank_admin(Request $request, $received_student_payment_id) {
       ...
       if ($request->ajax()) {			
          ...
          ->update(['confirm_by_bank_admin' => 1]);
                          ...
                          //$received_amount = ReceivedStudentPayment::select('received_amount_by_bank', 'student_id')->where('received_student_payment_id', $received_student_payment_id)->first();
                          $received_amount = DB::table('tbl_received_student_payments')->select('tbl_received_student_payments.received_amount_by_bank', 'tbl_received_student_payments.student_id', 'tbl_students.cash_in_hands')->join('tbl_students', 'tbl_students.student_id', '=', 'tbl_received_student_payments.student_id')->where('tbl_received_student_payments.received_student_payment_id', $received_student_payment_id)->first();
//                          echo '<pre>';
//                    print_r($this->data['paymentReport']);
//                  echo '</pre>';exit();
                          
                          DB::table('tbl_students')
            ->where('student_id', $received_amount->student_id)
            ->update(['cash_in_hands' => (float)$received_amount->cash_in_hands + (float)$received_amount->received_amount_by_bank]);
	...
	public function edit(Request $request, $receiptNo){
		 ...
		 if (!$request->ajax()) {
			...
			$this->data['paymentReport'] = DB::select('call get_receipt_details_by_receipt_no_bank_edit("' . $receiptNo . '" , "'.Auth::user()->role_id.'")');
			 //  echo '<pre>';
                 //   print_r($this->data['paymentReport']);
                 // echo '</pre>';exit();
